...
export default function PostPage() {
  ...
  const [savedPosts, save] = useLocalStorage("saved-posts", {});
  const router = useRouter();

  const [p, setPost] = useState<Post>();

  useEffect(() => {//First Network
    axios
  ...
  useEffect(() => {
    if (!loading && !p && !isOnline) {
      const saved = (Object.values(savedPosts) as [Post]).find((e)=>e?.slug == router.query.id)
        if(saved){
            setPost(saved);
        }
    }
  ...
  return (
    <article className="h-screen bg-white dark:bg-gray-800 pb-12 text-black dark:text-white">
      <div className="text-md ml-2 underline font-semibold">
        <Link href={"/"}>Volver</Link>
      </div>
      <div className="max-w-xl  justify-center mx-auto px-4">
        {!p && <h1>Post no disponible</h1>}
        {
            post && <>
         {/* Author & save button */}
        <div className="flex flex-row justify-between">
                    ...
                    <h3 className="flex gap-1 text-gray-600 dark:text-gray-200 text-sm font-light">
                     ...
                     { !saved && p &&  <div onClick={()=>save({...savedPosts, [p.id]: p })} className="cursor-pointer" title="Guardar">
                        <svg
                            width={28}
                            height={28}
                            viewBox="0 0 24 24"
                      ...
                      { saved && p  && <div onClick={()=>save({...savedPosts, [p.id]: undefined })} className="cursor-pointer" title="Remover">
                      <svg
                        width={28}
                        height={28}
                        viewBox="0 0 24 24"
        ...
        {/* Body */}
        <p className="space-y-2 text-gray-500 dark:text-gray-300 text-justify">{p?.body}</p>
        {/* Details (date) */}
